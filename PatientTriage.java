...
public class PatientTriage {
  ...
  /**
    ...
    public Patient remove(Time currentTime) throws NullPointerException, EmptyQueueException, BoundaryViolationException 
    {
    	...
    	Patient fPatient = timeHeap.peek();
    	Comparator<Time> timeComparator = new TimeComparator();
    	
    	if ((timeComparator.compare(fPatient.getArrivalTime().elapsed(currentTime), this.getMaxWait()) >= 0)) //If the elapsed time is more than the maxWait, prioritize the first of the timeHeap
    	{
    	    ...
    	    {
    	    	int fPatientPriPos = fPatient.getPriorityPos();
    	    	priorityHeap.remove(fPatientPriPos);
    	    }
    		timeHeap.poll();
    	}
    	else
    	{
    		fPatient = priorityHeap.peek();
    		if (timeHeap.size() == 1)
    		{
    			timeHeap.poll();
    		}
    		...
    		{
    			timeHeap.remove(fPatient.getTimePos());
    		}
    		priorityHeap.poll();
    	}
